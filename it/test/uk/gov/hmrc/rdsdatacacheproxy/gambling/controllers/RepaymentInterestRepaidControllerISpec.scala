...
class RepaymentInterestRepaidControllerISpec extends AnyWordSpec with Matchers with ScalaFutures with IntegrationPatience with ApplicationWithWiremock {

  class RepaymentInterestRepaidRdsStub extends RepaymentInterestRepaidDataSource {
    override def getRepaymentInterestRepaid(regime: Regime, regNumber: String, pageNo: Int, pageSize: Int) =
      Future {
        RepaymentInterestRepaidStubData.getRepaymentInterestRepaidData(regNumber, pageNo, pageSize)
      }
  }
  ...
  private final val endpoint = "/gambling/repayment-interest-repaid"
  private final val GBD = "gbd"

  "GET /gambling/repayment-interest-repaid (stubbed repo, no DB)" should {

    "return 200 with correct RepaymentInterestRepaidData" in {
      AuthStub.authorised()
      ...
      response.json.as[RepaymentInterestRepaid] mustBe getRepaymentInterestRepaidData("XGM00003122200")
    }

    "return 200 with correct RepaymentInterestRepaidData when pageNo & pageSize NOT provided" in {
      AuthStub.authorised()
