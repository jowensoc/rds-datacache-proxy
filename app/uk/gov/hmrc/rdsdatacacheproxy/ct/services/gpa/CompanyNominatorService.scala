...
class CompanyNominatorService @Inject() (companyNominatorRepository: CompanyNominatorRepository) extends Logging {

  def getIsCompanyNominatorOfGPA(gpaUtr: Long, nominatedCompanyUtr: Long): Future[CompanyNominator] = {
    logger.info(s"Calling repository for gpaUtr: $gpaUtr and nominatedCompanyUtr: $nominatedCompanyUtr")

    companyNominatorRepository.getIsCompanyNominatorOfGPA(gpaUtr, nominatedCompanyUtr)
  }
}
